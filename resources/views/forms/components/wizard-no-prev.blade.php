...
<div x-data="{
    ...
    }" x-on:next-wizard-step.window="if ($event.detail.statePath === '{{ $getStatePath() }}') nextStep()" x-cloak {!! $getId() ? "id=\" {$getId()}\"" : null !!} {{ $attributes->merge($getExtraAttributes())->class(['filament-forms-wizard-component grid gap-y-6']) }} {{ $getExtraAlpineAttributeBag() }}>
    ...
    <ol {!! $getLabel() ? 'aria-label="' . $getLabel() . '"' : null !!} role="list" @class([ 'filament-forms-wizard-component-header border border-gray-300 shadow-sm bg-white rounded-xl overflow-hidden divide-y divide-gray-300 md:flex md:divide-y-0' , 'dark:bg-gray-800 dark:border-gray-700 dark:divide-gray-700'=> config('forms.dark_mode'),
        ])
        >
        @foreach ($getChildComponentContainer()->getComponents() as $step)
        <li class="filament-forms-wizard-component-header-step relative overflow-hidden group md:flex-1">
            <button type="button" x-on:click="if (isStepAccessible(step, {{ $loop->index }})) step = '{{ $step->getId() }}'" x-bind:aria-current="getStepIndex(step) === {{ $loop->index }} ? 'step' : null" x-bind:class="{
                ...
                <div class="flex items-center gap-3 px-5 py-4 text-sm font-medium">
                    <div class="flex-shrink-0">
                        <div x-bind:class="{
                            ...
                            @if ($step->getIcon())
                            <x-dynamic-component :component="$step->getIcon()" x-show="getStepIndex(step) <= {{ $loop->index }}" x-cloak x-bind:class="{
                            ...
                            @else
                            <span x-show="getStepIndex(step) <= {{ $loop->index }}" x-bind:class="{
                                            'text-gray-500 @if (config('forms.dark_mode')) dark:text-gray-400 @endif': getStepIndex(step) !== {{ $loop->index }},
                                            'text-primary-500': getStepIndex(step) === {{ $loop->index }},
                                        }">
                    ...
                    <div class="flex flex-col items-start justify-center">
                        ...
                        <div @class([ 'filament-forms-wizard-component-header-step-description text-sm leading-4 font-medium text-gray-500' , 'dark:text-gray-400'=> config('forms.dark_mode'),
                            ])>
                            {{ $description }}
                        </div>
                        @endif
            ...
            <div class="absolute inset-0 top-0 left-0 hidden w-3 md:block" aria-hidden="true">
                <svg @class([ 'h-full w-full text-gray-300 rtl:rotate-180' , 'dark:text-gray-700'=> config('forms.dark_mode'),
    ...
    <div class="flex items-center justify-between">
        ...
        <div>
            <x-forms::button :icon="$nextArrowIcon" icon-position="after" x-show="! isLastStep()" x-cloak x-on:click="$wire.dispatchFormEvent('wizard::nextStep', '{{ $getStatePath() }}', getStepIndex(step))" wire:loading.class.delay="opacity-70 cursor-wait" size="sm">
